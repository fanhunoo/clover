/*
	...
	@Time: 2018-07-23
	@Tittle: bodyTab
	@Description: 点击菜单添加标签页
	@Import:
*/
BodyTab = function () {
    var tabFilter,menu=[],liIndex,curNav,delMenu,
        changeRefreshStr = window.sessionStorage.getItem("changeRefresh");//配置：是否刷新
    ...
    this.navBar = function(strData){
        ...
        $.each(data,function(i,obj){
            ...
            ulHtml += '<a>';
            //TODO:  icon ??
            if(util.isNotNull(obj.icon)){
                if(obj.icon.indexOf("icon-") !== -1){
                    ulHtml += '<i class="seraph '+obj.icon+'" data-icon="'+obj.icon+'"></i>';
                }else{
                    ulHtml += '<i class="layui-icon" data-icon="'+obj.icon+'">'+obj.icon+'</i>';
                }
            }
            //检查是否有子菜单
            var childData = JSON.parse(obj.childJson);
            if(util.isNull(childData)){
                ulHtml += '<cite>'+obj.name+'</cite></a>';
            }else{
                ulHtml += '<cite>'+obj.name+'</cite>';
                ulHtml += '<span class="layui-nav-more"></span>';
                ulHtml += '</a>';
                ulHtml += '<dl class="layui-nav-child">';
                $.each(childData,function(j,child){
                    if(child.target === "_blank"){
                        ulHtml += '<dd><a data-url="'+'.'+child.url+'" target="'+child.target+'">';
                    }else{
                        ulHtml += '<dd><a data-url="'+'.'+childData[j].url+'">';
                    }
                    //TODO:  icon ??
                    if(util.isNotNull(child.icon)){
                        if(child.icon.indexOf("icon-") !== -1){
                            ulHtml += '<i class="seraph '+child.icon+'" data-icon="'+child.icon+'"></i>';
                        }else{
                            ulHtml += '<i class="layui-icon" data-icon="'+child.icon+'">'+child.icon+'</i>';
                        }
                    }
    ...
    //是否点击窗口切换刷新页面
    this.changeRegresh = function(index){
    ...
    this.tabAdd = function(openTab){
        if(window.sessionStorage.getItem("menu")){
            menu = JSON.parse(window.sessionStorage.getItem("menu"));
        }
        var openTabNum = _this.tabConfig.openTabNum;
        tabFilter = _this.tabConfig.tabFilter;

        if(openTab.attr("target") == "_blank"){
            window.open(openTab.attr("data-url"));
        }else if(util.isNotNull(openTab.attr("data-url"))){
            var title = '';
            if(util.isNotNull(openTab.find("i.seraph,i.layui-icon").attr("data-icon"))){
                if(util.isNotNull(openTab.find("i.seraph").attr("data-icon") )){
                    title += '<i class="seraph '+openTab.find("i.seraph").attr("data-icon")+'"></i>';
                }else{
                    title += '<i class="layui-icon">'+openTab.find("i.layui-icon").attr("data-icon")+'</i>';
                }
            }
            //已打开的窗口中不存在
            if(_this.hasTab(openTab.find("cite").text()) == -1 && openTab.siblings("dl.layui-nav-child").length == 0){
                if($(".layui-tab-title.top_tab li").length >= openTabNum){
                    layer.msg('只能同时打开'+openTabNum+'个选项卡哦。不然系统会卡的！');
                    return;
                }
                tabIdIndex++;
                title += '<cite>'+openTab.find("cite").text()+'</cite>';
                title += '<i class="layui-icon layui-unselect layui-tab-close" data-id="'+tabIdIndex+'">&#x1006;</i>';
                layuiElement.tabAdd(tabFilter, {
                    title : title,
                    content :"<iframe src='"+openTab.attr("data-url")+"' data-id='"+tabIdIndex+"'></iframe>",
                    id : new Date().getTime()
                });
                //当前窗口内容
                var curmenu = {
                    "icon" : util.isNotNull(openTab.find("i.seraph").attr("data-icon")) ? openTab.find("i.seraph").attr("data-icon") : openTab.find("i.layui-icon").attr("data-icon"),
                    "title" : openTab.find("cite").text(),
                    "href" : openTab.attr("data-url"),
                ...
                layuiElement.tabChange(tabFilter, _this.getLayId(openTab.find("cite").text()));
                _this.tabMove(); //顶部窗口是否可滚动
            }else{
                //当前窗口内容
                var curmenu = {
                    "icon" : openTab.find("i.seraph").attr("data-icon")!=undefined ? openTab.find("i.seraph").attr("data-icon") : openTab.find("i.layui-icon").attr("data-icon"),
                    "title" : openTab.find("cite").text(),
                    "href" : openTab.attr("data-url")
                };
                _this.changeRegresh(openTab.parent('.layui-nav-item').index());
                window.sessionStorage.setItem("curmenu", JSON.stringify(curmenu));  //当前的窗口
                element.tabChange(tabFilter, _this.getLayId(openTab.find("cite").text()));
                _this.tabMove(); //顶部窗口是否可滚动
            }
        }
    };
